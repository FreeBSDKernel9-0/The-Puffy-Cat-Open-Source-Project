var CUSTOM_PARAMETERS = {
    ...
    resize_window_callback: function() {
        ...
        var e, n, t = document.getElementById("app-container"),
            o = document.getElementById("canvas"),
            r = window.innerWidth,
            i = +window.innerHeight;
        -1 == r && -1 == i || (e = 1e3, n = 1500, e = r, n = i, r = 1, r = window.devicePixelRatio || 1, t.style.width = e + "px", t.style.height = 0 + n + "px", o.width = Math.floor(e * r), o.height = Math.floor(n * r))
    }
},
FileLoader = {
    options: {
    ...
    request: function(e, s, a, d) {
        ...
        var l = {
            send: function() {
                var n = this.onprogress,
                    t = this.onload,
                    o = this.onerror,
                    r = this.onretry,
                    i = new XMLHttpRequest;
                i._loadedSize = 0, i.open(s, e, !0), i.responseType = a, i.onprogress = function(e) {
                    n && n(i, e, i._loadedSize), i._loadedSize = e.loaded
                }, i.onerror = function(e) {
                    d == FileLoader.options.retryCount ? o && o(i, e) : (r && r(i, e, i._loadedSize, d), i._loadedSize = 0, d += 1, setTimeout(l.send.bind(l), FileLoader.options.retryInterval))
                }, i.onload = function(e) {
                    t && t(i, e)
                }, i.send(null)
            }
    ...
    size: function(e, o) {
        var n = FileLoader.request(e, "HEAD", "text");
        n.onerror = function(e, n) {
            o(void 0)
        }, n.onload = function(e, n) {
            var t;
            4 === e.readyState && (200 === e.status ? (t = e.getResponseHeader("content-length"), o(t)) : o(void 0))
        }, n.send()
    },
    load: function(o, r, i, s, a, d) {
        var e = FileLoader.request(o, "GET", r);
        e.onprogress = function(e, n, t) {
            ...
            var t;
            e.readyState === XMLHttpRequest.DONE && (200 === e.status ? (t = e.response, a("json" == r && "string" == typeof t ? JSON.parse(t) : t)) : s("Error loading '" + o + "' (" + n + ")"))
...
EngineLoader = {
    ...
    loadAndInstantiateWasmAsync: function(e, n, t) {
        FileLoader.load(e, "arraybuffer", function(e) {
            ...
            if (e.byteLength != EngineLoader.wasm_size) throw "Invalid wasm size. Expected: " + EngineLoader.wasm_size + ", actual: " + e.byteLength;
    ...
    streamAndInstantiateWasmAsync: async function(n, t, o) {
        ...
        "function" == typeof TransformStream && ReadableStream.prototype.pipeThrough && (e = async function(e) {
            var n, t = await fetch(e);
            return t.ok ? (n = new TransformStream({
                transform(e, n) {
                    ProgressUpdater.updateCurrent(e.byteLength), n.enqueue(e)
                }
            }), new Response(t.body.pipeThrough(n), t)) : new Response(null, t)
            ...
            console.log("wasm streaming instantiation failed! " + e), console.log("Fallback to wasm loading"), EngineLoader.loadAndInstantiateWasmAsync(n, t, o)
        })
    },
    loadWasmAsync: function(t) {
        Module.instantiateWasm = function(e, n) {
            return EngineLoader.stream_wasm && "function" == typeof WebAssembly.instantiateStreaming ? EngineLoader.streamAndInstantiateWasmAsync(t + ".wasm", e, n) : EngineLoader.loadAndInstantiateWasmAsync(t + ".wasm", e, n), {}
...
GameArchiveLoader = {
    ...
    onReceiveDescription: function(e) {
        var n = e.total_size,
            t = CUSTOM_PARAMETERS.exe_name,
            o = (this._files = e.content, Module.isWASMSupported);
        o ? (EngineLoader.loadWasmAsync(t), n += EngineLoader.wasm_size + EngineLoader.wasmjs_size) : (EngineLoader.loadAsmJsAsync(t), n += EngineLoader.asmjs_size), this.downloadContent(), ProgressUpdater.resetCurrent(), o && EngineLoader.updateWasmInstantiateProgress(n), ProgressUpdater.setupTotal(n + EngineLoader.wasm_instantiate_progress)
    },
    ...
    downloadPiece: function(n, e) {
        if (e < n.lastRequestedPiece) throw RangeError("Request out of order: " + n.name + ", index: " + e + ", last requested piece: " + n.lastRequestedPiece);
        var t = n.pieces[e],
            o = (n.lastRequestedPiece = e, n.totalLoadedPieces = 0, this._archiveLocationFilter("/" + t.name));
        FileLoader.load(o, "arraybuffer", function(e) {
            GameArchiveLoader.notifyDownloadProgress(e)
        }, function(e) {
            GameArchiveLoader.notifyFileDownloadError(e)
        }, function(e) {
            t.data = new Uint8Array(e), t.dataLength = t.data.length, total = t.dataLength, downloaded = t.dataLength, GameArchiveLoader.onPieceLoaded(n, t), t.data = void 0
        }, function(e, n) {
            ProgressUpdater.updateCurrent(-e)
        })
    },
    ...
    onPieceLoaded: function(e, n) {
        var t;
        this.addPieceToFile(e, n), ++e.totalLoadedPieces, e.totalLoadedPieces == e.pieces.length ? this.onFileLoaded(e) : (t = e.lastRequestedPiece + 1) < e.pieces.length && this.downloadPiece(e, t)
    },
    verifyFile: function(e) {
        ...
        if (1 < e.pieces.length)
            for (var o = e.pieces, t = 0; t < o.length; ++t) {
                ...
                if (0 < t) {
                    var s = o[t - 1];
                    if (s.offset + s.dataLength > i) throw RangeError("Segment underflow in file: " + e.name + ", offset: " + (s.offset + s.dataLength) + " , start: " + i)
                }
                if (o.length - 2 > t) {
                    s = o[t + 1];
                    if (r > s.offset) throw RangeError("Segment overflow in file: " + e.name + ", offset: " + s.offset + ", end: " + r)
                }
            }
    },
...
ProgressView = {
    ...
    addProgress: function(e) {
        e.insertAdjacentHTML("afterend", '<div id="' + ProgressView.progress_id + '" class="canvas-app-progress"><div id="' + ProgressView.bar_id + '" class="canvas-app-progress-bar" style="transform: scaleX(0.0);"></div></div>'), ProgressView.bar = document.getElementById(ProgressView.bar_id), ProgressView.progress = document.getElementById(ProgressView.progress_id)
    },
...
ProgressUpdater = {
    ...
    setCurrent: function(e) {
        this.current = e;
        var n = this.calculateProgress();
        ProgressView.updateProgress(n), this.notifyListeners(n)
    },
...
Module = {
    ...
    prepareErrorObject: function(e, n, t, o, r) {
        var i = (n = void 0 === n ? "" : n) + ":" + (t = void 0 === t ? 0 : t) + ":" + (o = void 0 === o ? 0 : o),
            s = r || (void 0 !== window.event ? window.event.error : "") || e || "Undefined Error",
            a = "",
            d = "",
            s = ("object" == typeof s && void 0 !== s.stack && void 0 !== s.message ? (d = String(s.stack), a = String(s.message)) : (a = (d = String(s).split("\n")).shift(), d = d.join("\n")), d = d || i, /at (\S+:\d*$)/.exec(a));
        return s && (a = a.replace(/(at \S+:\d*$)/, ""), d = s[1] + "\n" + d), a = a.replace(/(abort\(.+\)) at .+(?=)/, "$1"), {
    ...
    runApp: function(e, n) {
        ...
        var t = CUSTOM_PARAMETERS.full_screen_container;
        "string" == typeof t && (t = document.querySelector(t)), Module.fullScreenContainer = t || Module.canvas, Module.hasWebGLSupport() ? (Module.canvas.focus(), CUSTOM_PARAMETERS.disable_context_menu && (Module.canvas.oncontextmenu = function(e) {
            e.preventDefault()
        }), Module._preloadAndCallMain()) : (ProgressUpdater.complete(), Module.setStatus = function(e) {
    ...
    preSync: function(n) {
        1 != Module.persistentStorage ? n() : FS.syncfs(!0, function(e) {
            e ? (Module._syncTries += 1, console.warn("Unable to synchronize mounted file systems: " + e), Module._syncMaxTries > Module._syncTries ? Module.preSync(n) : (Module._syncInitial = !0, n())) : (Module._syncInitial = !0, void 0 !== n && n())
        })
    },
    preloadAll: function() {
    ...
    preInit: [function() {
        ...
        try {
            ...
            FS.close = function(e) {
                var n = t(e);
                return Module.persistentSync(), n
            }
        } catch (e) {
            return Module.persistentStorage = !1, void Module._preloadAndCallMain()
        }
    ...
    _startSyncFS: function() {
        Module._syncInProgress = !0, Module._syncMaxTries > Module._syncTries && FS.syncfs(!1, function(e) {
            Module._syncInProgress = !1, e && (console.warn("Unable to synchronize mounted file systems: " + e), Module._syncTries += 1), Module._syncNeeded && (Module._syncNeeded = !1, Module._startSyncFS())
        })
    }
};
Module.persistentStorage = "undefined" != typeof window && !(!window.indexedDB && !window.mozIndexedDB && !window.webkitIndexedDB && !window.msIndexedDB), Module.INITIAL_MEMORY = CUSTOM_PARAMETERS.custom_heap_size, Module.onRuntimeInitialized = function() {
    ...
    return n + (e = "dmengine.wasm" != e && "dmengine_release.wasm" != e && "dmengine_headless.wasm" != e ? e : "puffy_cat.wasm")
}, window.onerror = function(e, n, t, o, r) {
    var i;
    void 0 !== Module.ccall && (i = Module.prepareErrorObject(e, n, t, o, r), Module.ccall("JSWriteDump", "null", ["string"], [JSON.stringify(i.stack)])), Module.setStatus("Exception thrown, see JavaScript console"), Module.setStatus = function(e) {
